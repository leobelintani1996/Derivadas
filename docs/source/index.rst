...
computacionais, que estão sendo utilizadas nos principais algoritmos de inteligência artificial do mundo todo.
O crescimento relacionado a área de machine learning(aprendizado de máquina) e deep learning(aprendizado profundo) muito se deve aos constantes esforços tecnológicos das iniciativas publico e privadas 
de diversas esferas da sociedade mundial. Estes esforços não poderiam resultar em algo diferente a não ser um grandioso sucesso dos envolvidos. Podemos atribuir boa parte deste sucesso aos algoritmos de otimização teorizados e implementados, 
...
por exemplo, buscam minimizar uma função conhecida como custo e esta função é minimizada através de técnicas como o gradiente descendente (utilizado para atualizar os parâmetros de uma rede neural) e a retro propagação (utilizado para retro propagar a função custo), 
